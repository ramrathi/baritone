    ...
    for fmt in fmts:
        ...
        for audiosource in testsources:
            ...
            test_file_name = "%s_%s_%ssec.%s" % (
                os.path.join(dest, filename), starttime, nseconds,
                extension.replace(".", "")
            )
            ...
            subprocess.check_output(
                [
                    "ffmpeg", "-y", "-ss",
                    "%d" % starttime, '-t',
                    "%d" % nseconds, "-i", audiosource, test_file_name
                ]
            )
...
class DejavuTest(object):
    def __init__(self, folder, seconds):
        ...
        self.test_files = [
            ...
            if os.path.isfile(os.path.join(self.test_folder, f)) and
